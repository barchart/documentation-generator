	...
	for (const param of parameters) {
		...
		if (paramSchema) {
			param.exampleValues.object =
				param.example ||
				param.default ||
				openAPIHelper.getSample(paramSchema, options, { skipReadOnly: true, quiet: true }, api);
		...
		if (param.in === 'cookie') {
			context.cookieParameters.data.push(param);
		}
		if (param.in === 'header') {
			context.headerParameters.data.push(param);
		}
		if (param.in === 'path') {
			context.pathParameters.data.push(param);
		}
		if (param.in === 'query') {
			context.queryString.data.push(param);
		}
...
function _getResponse(response, code, api, isCustomHeader = false) {
	...
	context.description = typeof response.description === 'string' ? response.description.trim() : undefined;
	context.content = {};
	context.status = code;
	...
	if (ref) {
		...
		if (response.headers) {
			Object.keys(response.headers).forEach((h) => {
				...
				entryHeader.name = h;
				entryHeader.description = header.description;
				...
				if (headerSchema) {
					entryHeader.exampleValues.object =
						entryHeader.example ||
						entryHeader.default ||
						openAPIHelper.getSample(headerSchema, options, { skipReadOnly: true, quiet: true }, api);
					...
					if (entryHeader.description)
						entryHeader.description = entryHeader.description.replace(/\n/gm, '<br>');
				}
...
function resolveSchema(schema, api, isCustomHeader) {
	...
	function parseSchema(schema, name = undefined, parent = undefined, skip = false) {
		let nameN = name;
		...
		switch (schema.type) {
			case 'object': {
				...
				if (ref) {
					...
					const t = isCustomHeader ? `[<code>${toTitleCase(type)}</code>](${contentLink}${link})` : `[<code>${toTitleCase(type)}</code>](#${link})`;
					context.data.push({
						name: nameN,
			...
			case 'array': {
				...
				if (items) {
					...
					if (ref) {
						const link = ref.replace('#/components/', '').replace(/\//g, '');
						const type = ref.replace('#/components/schemas/', '');
						const t = isCustomHeader ? `[<code>Array&lt;${toTitleCase(type)}&gt;</code>](${contentLink}${link})` : `[<code>Array&lt;${toTitleCase(type)}&gt;</code>](#${link})`;
			...
			default: {
				const example = schema.example || openAPIHelper.getSample(schema, options, {}, api);
				const ref = schema.$ref;
	...
	switch (securityType) {
		case 'http': {
			if (security.scheme === 'bearer') {
				...
				context.description = security.description;
				context.header = {
					headers: [
						{ title: 'name', length: 4 },
				...
				Object.keys(flows).forEach((flowType) => {
					...
					if (scopesData) {
						...
						Object.keys(scopesData).forEach((scopeName) => {
							flow.scopes.push({
								title: scopeName,
								description: scopesData[scopeName]
							});
						});
					}
