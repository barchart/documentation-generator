		...
		switch (schema.type) {
			case 'object': {
				...
				if (ref) {
					...
					const t = isCustomHeader
						? `[<code>${toTitleCase(type)}</code>](${contentLink}${link})`
						: `[<code>${toTitleCase(type)}</code>](#${link})`;
					context.data.push({
						name: nameN,
			...
			case 'array': {
				...
				if (items) {
					...
					if (ref) {
						...
						const type = ref.replace('#/components/schemas/', '');
						const t = isCustomHeader
							? `[<code>Array&lt;${toTitleCase(type)}&gt;</code>](${contentLink}${link})`
							: `[<code>Array&lt;${toTitleCase(type)}&gt;</code>](#${link})`;
