...
const HTMLParser = require('../../../html/HTMLParser');
...
exports.prepareDescription = prepareDescription;
...
function convertToMD(str) {
	if (typeof str === 'string') {
		...
		return htmlParser.parseFromString(str)
			.replace(/\n/g, '<br>')
			.replace(/\|/g, '\\|');
	} else {
		return str;
...
function getParameters(parameters, opt) {
	...
	const options = opt.data.root['options'];
	const headers = [
		{ title: 'name', length: 4 },
		{ title: 'type', length: 4 },
	...
	for (const param of parameters) {
		const parsedParameter = parseParameter(param, api, options);
...
function parseParameter(parameter, api, opt = {}) {
	...
	if (ref) {
		...
		if (paramSchema) {
			param.exampleValues.object = param.example || param.default || openAPIHelper.getSample(paramSchema, options, {
				skipReadOnly: true,
				quiet: true,
				skipNonRequired: !!opt.ignoreOptional
			}, api);
...
function getResponses(res, isCustomHeader, opt) {
	...
	Object.keys(res).forEach((code) => {
		...
		const entry = _getResponse(response, code, api, isCustomHeader, options);
...
function _getResponse(response, code, api, isCustomHeader = false, opt = {}) {
	const options = { sample: true };
	const context = {};
	const headers = [
		{ title: 'name', length: 4 },
	...
	if (ref) {
		...
		if (response.content) {
			Object.keys(response.content).forEach((ct) => {
				...
				if (contentType.schema && contentType.schema.type && contentType.schema.type !== 'object' && contentType.schema.type !== 'array') {
					...
					context.content[ct] = resolveSchema(contentType.schema, api, isCustomHeader, opt);
				}

				if (contentType.schema.oneOf || contentType.schema.anyOf || contentType.schema.allOf) {
					context.of = processOfSchemas(contentType.schema, api, isCustomHeader, opt);
				} else {
					context.content[ct] = resolveSchema(contentType.schema, api, isCustomHeader, opt);
				}
			});
		...
		if (response.headers) {
			Object.keys(response.headers).forEach((h) => {
				...
				if (headerSchema) {
					entryHeader.exampleValues.object = entryHeader.example || entryHeader.default || openAPIHelper.getSample(headerSchema, options, {
						skipReadOnly: true,
						quiet: true,
						skipNonRequired: !!opt.ignoreOptional
					}, api);
...
function processOfSchemas(schema, api, isCustomHeader, options) {
	const context = {
		oneOf: { name: 'One of', data: [] },
		anyOf: { name: 'Any of', data: [] },
		allOf: { name: 'All of', data: [] }
	};

	Object.keys(schema).forEach((key) => {
		schema[key].forEach((data) => {
			if (data.$ref) {
				const link = data.$ref
					.replace('#/components/', '')
					.replace(/\//g, '')
					.toLowerCase();
				const name = data.$ref.replace('#/components/schemas/', '');

				const ref = {};

				ref.link = `[${name}](#${link})`;
				ref.oLink = `[${name}](/content/api/components?id=${link})`;

				context[key].data.push(ref);
			} else {
				const resolvedSchema = resolveSchema(data, api, isCustomHeader, options);

				context[key].data.push(resolvedSchema);
			}
		});
	});
...
function getResponse(response, isCustomHeader, opt) {
	...
	const api = opt.data.root[0];
	const options = opt.data.root['options'];
	const context = _getResponse(response, undefined, api, isCustomHeader, options);
...
function getRequestBody(req, isCustomHeader, opt) {
	...
	Object.keys(contents).forEach((ct) => {
		...
		if (content.schema.oneOf || content.schema.anyOf || content.schema.allOf) {
			const of = processOfSchemas(content.schema, api, isCustomHeader, options);
			context[ct] = { of: of };
		} else {
			context[ct] = resolveSchema(content.schema, api, isCustomHeader, options);
		}
	});
...
function resolveSchema(schema, api, isCustomHeader, opt) {
	const context = {
		headers: [
			{ title: 'name', length: 4 },
			{ title: 'type', length: 4 },
	...
	function parseSchema(sch, name = undefined, parent = undefined, skip = false) {
		...
		default: {
			const example = sch.example || openAPIHelper.getSample(sch, options, { skipNonRequired: !!opt.ignoreOptional }, api);
			const ref = sch.$ref;
	...
	const example = openAPIHelper.getSample(schema, options, { skipReadOnly: true, quiet: true, skipNonRequired: !!opt.ignoreOptional }, api);
...
function getSchema(schema, opt) {
	const api = opt.data.root[0];
	const options = opt.data.root['options'];
	const context = resolveSchema(schema, api, false, options);
...
function getFilename(options) {
	if (os.platform() === 'win32') {
		return options.data.root.options.filename.replace(/\\/g, '/');
	} else {
