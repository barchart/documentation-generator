		...
		if (response.content) {
			Object.keys(response.content).forEach((ct) => {
				...
				if (contentType.schema.oneOf || contentType.schema.anyOf || contentType.schema.allOf) {
					context.of = processOfSchemas(contentType.schema, api, isCustomHeader);
				} else {
					context.content[ct] = resolveSchema(contentType.schema, api, isCustomHeader);
				}
			});
...
function processOfSchemas(schema, api, isCustomHeader) {
	const context = {
	...
	Object.keys(schema).forEach((key) => {
		schema[key].forEach((data) => {
			if (data.$ref) {
				...
				const resolvedSchema = resolveSchema(data, api, isCustomHeader);
...
function getRequestBody(req, isCustomHeader, opt) {
	...
	Object.keys(contents).forEach((ct) => {
		...
		if (content.schema.oneOf || content.schema.anyOf || content.schema.allOf) {
			const of = processOfSchemas(content.schema, api, isCustomHeader);
			context[ct] = { of: of };
		} else {
