...
function getImport(obj) {
	...
	return `${global.packageName}${path.replace(executionPath, '').replace(/\\/g, '/')}/${obj.meta.filename.replace('.js', '').replace(/\\/g, '/')}`;
}
...
function getFile(obj) {
	...
	return `${path.replace(executionPath, '').replace(/\\/g, '/')}/${obj.meta.filename.replace(/\\/g, '/')}`;
}
