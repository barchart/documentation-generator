...
module.exports = (() => {
	class JSDocGenerator extends BaseGenerator {
		constructor(options) {
			super(options);
		}
		...
		_generate() {
			return Promise.resolve()
				.then(async () => {
					const resolvedPath = await this.resolvePath('jsdocPath', this.options.jsdocPath);

					if (resolvedPath.isSourceExist) {
						console.info(chalk.grey(`Scanning ${resolvedPath.path} for JSDoc...`));
						console.info(chalk.grey('Generating JSDoc documentation...'));

						return generateDocumentation({
							docsFolder: this.options.docsFolder,
							jsdocPath: resolvedPath.path,
							packageName: this.options.projectName
						});
					} else {
						throw new Error('The path to the source code not found. JSDoc documentation skipped.');
					}
				});
		}
	}
	 ...
	 * @param {String} meta.jsdocPath - a path to the source code
	 * @returns {Promise}
	 */
	function generateDocumentation(meta) {
		...
		return jsdoc2md.clear().then(() => {
			return jsdoc2md
				.getTemplateData({
					files: [
						path.resolve(meta.jsdocPath, '**', '{.,}*.{js,ts}')
					],
					configure: path.resolve(__dirname, '../../../jsdoc2md.js')
				})
				.then((templateData) => {
					return templateData.reduce(
						(templateGroups, identifier) => {
							...
							const splitJSDocPath = meta.jsdocPath.split(isWindows ? '\\' : '/');
							const folder = splitJSDocPath[splitJSDocPath.length - 1];
							const arrayFilePath = templatePath.split(folder);
							const filePath = `${folder}` + `${isWindows ? arrayFilePath[1].replace(/\\/g, '-') : arrayFilePath[1].replace(/\//g, '-')}`;
				...
				.then((templateGroups) => {
					...
					keys.forEach((filePath) => {
						...
						const output = jsdoc2md.renderSync({
							...
							template,
							separators: true,
							plugin: `${path.resolve(__dirname, 'dmd-plugin/lib/barchart-dmd-plugin.js')}`,
					...
					const sidebarSDKFolder = sidebar.replace(/(<!-- sdk_open -->(\s|.)*<!-- sdk_close -->)/gm, sdkSidebar);
