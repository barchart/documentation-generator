...
function writeFileSync(filePath, data, options) {
	if (isWindows()) {
		writeFileSyncWin(filePath, data, options);
	} else {
		writeFileSyncUnix(filePath, data, options);
	}
}

function writeFileSyncWin(filePath, data, options) {
	try {
...
function writeFileSyncUnix(filePath, data, options) {
	try {
		const pathSplit = filePath.split('/');
		const dir = path.join('/', ...pathSplit.slice(0, pathSplit.length - 1));
		const isDirExist = existsSync(dir);
...
function copyFileSync(src, dest, flags) {
	let destDir = dest.split(isWindows ? '\\' : '/');
	destDir.pop();
	destDir = destDir.join(isWindows ? '\\' : '/');
